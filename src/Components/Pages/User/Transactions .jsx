...
const Transactions = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axiosSecure.get('/transactions?status=complete')
                    ...
                    <tbody className='text-lg'>
                        {
                            transactions.map((transaction, idx) => <tr key={idx} className="hover">
                                ...
                                <td>
                                <p className='flex gap-2 items-center'><span className='text-green-600'><FaArrowRight></FaArrowRight></span>{transaction?.type}</p>
                                </td>
                                <td>
